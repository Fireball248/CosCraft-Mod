...
public class CommonEventHandler {
    ...
    public void onPlayerClone(PlayerEvent.Clone event) {
        ...
        if (oldCap.getAllomancyPower() >= 0) {
            ...
            Allomancy.network.sendTo(new AllomancyPowerPacket(oldCap.getAllomancyPower()), (EntityPlayerMP) event.getEntity());
        }
        if (event.getEntityPlayer().world.getGameRules().getBoolean("keepInventory") || !event.isWasDeath()) { // if keepInventory is true, or they didn't die, allow them to keep their metals, too
            for (int i = 0; i < 8; i++) {
                ...
                Allomancy.network.sendTo(new AllomancyCapabilityPacket(cap, event.getEntity().getEntityId()), (EntityPlayerMP) event.getEntity());
    ...
    public void onPlayerLogin(EntityJoinWorldEvent event) {
        if (event.getEntity() instanceof EntityPlayerMP) {
            ...
            Allomancy.network.sendTo(new AllomancyCapabilityPacket(cap, event.getEntity().getEntityId()), player);
            if (cap.getAllomancyPower() >= 0) {
                Allomancy.network.sendTo(new AllomancyCapabilityPacket(cap, player.getEntityId()), player);
                ...
                Allomancy.network.sendTo(new AllomancyPowerPacket(randomMisting), player);
                ItemStack dust = new ItemStack(Item.getByNameOrId("allomancy:flake_" + Allomancy.flakeMetals[randomMisting]));
