			...
			mainThread.addScheduledTask(new Runnable() {
				...
				public void run() {
					...
					if (player.getEntityWorld().isBlockLoaded(block) && (AllomancyUtils.isBlockMetal(ctx.getServerHandler().player.world.getBlockState(block).getBlock()) //Standard check
							|| (player.getHeldItemMainhand().getItem() == ModItems.COIN_BAG && (CoinBag.findArrow(player) != null || player.isCreative()) && message.direction == AllomancyUtils.PUSH))) { // Check for the coin bag
