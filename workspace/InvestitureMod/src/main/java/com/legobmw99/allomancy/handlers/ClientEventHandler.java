            ...
            if (cap.getAllomancyPower() >= 0) {
                ...
                if (this.mc.gameSettings.keyBindAttack.isKeyDown()) {
                    ...
                    if (cap.getMetalBurning(AllomancyCapability.IRON)) {
                        if (mov != null) {
                            if (mov.entityHit != null && AllomancyUtils.isEntityMetal(mov.entityHit)) {
                        		Allomancy.network.sendToServer(new TryPushPullEntity(mov.entityHit.getEntityId(), AllomancyUtils.PULL));
                            }
                            ...
                            if (mov.typeOfHit == RayTraceResult.Type.BLOCK) {
                                ...
                                if (AllomancyUtils.isBlockMetal(this.mc.world.getBlockState(bp).getBlock())) {
                                    Allomancy.network.sendToServer(new TryPushPullBlock(bp, AllomancyUtils.PULL));
                                }
                            }
                    ...
                    if (cap.getMetalBurning(AllomancyCapability.ZINC)) {
                        ...
                        if ((mov != null) && (mov.entityHit != null) && (mov.entityHit instanceof EntityCreature) && !(mov.entityHit instanceof EntityPlayer)) {
                            ...
                            Allomancy.network.sendToServer(new ChangeEmotionPacket(entity.getEntityId(), true));
                ...
                if (this.mc.gameSettings.keyBindUseItem.isKeyDown()) {
                    ...
                    if (cap.getMetalBurning(AllomancyCapability.STEEL)) {
                        if (mov != null) {
                            if (mov.entityHit != null && AllomancyUtils.isEntityMetal(mov.entityHit)) {
                                Allomancy.network.sendToServer(new TryPushPullEntity(mov.entityHit.getEntityId(), AllomancyUtils.PUSH));
                            ...
                            if (mov.typeOfHit == RayTraceResult.Type.BLOCK ) {
                                ...
                                BlockPos bp = mov.getBlockPos();
                                if (AllomancyUtils.isBlockMetal(this.mc.world.getBlockState(bp).getBlock()) || (player.getHeldItemMainhand().getItem() == ModItems.COIN_BAG && player.isSneaking())) {
                                    Allomancy.network.sendToServer(new TryPushPullBlock(bp, AllomancyUtils.PUSH));
                    ...
                    if (cap.getMetalBurning(AllomancyCapability.BRASS)) {
                        ...
                        if ((mov != null) && (mov.entityHit != null) && (mov.entityHit instanceof EntityCreature) && !(mov.entityHit instanceof EntityPlayer)) {
                            entity = mov.entityHit;
                            Allomancy.network.sendToServer(new ChangeEmotionPacket(entity.getEntityId(), false));
                ...
                if (cap.getMetalBurning(AllomancyCapability.BRONZE) && !cap.getMetalBurning(AllomancyCapability.COPPER)) {
                    ...
                    for (Entity curEntity : eListBurners) {
                        if (curEntity != null && curEntity != player && curEntity.hasCapability(Allomancy.PLAYER_CAP, null)) {
                            Allomancy.network.sendToServer(new GetCapabilitiesPacket(curEntity.getEntityId()));
                ...
                for (EntityPlayer entity : metalBurners) {
                    Allomancy.network.sendToServer(new GetCapabilitiesPacket(entity.getEntityId()));
